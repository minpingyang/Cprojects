...
void saveTicGame(int sizeF, char humanChar, int humanRow, int humanCol){
    ...
    fprintf(fp, "%d %c %d %d",sizeF,humanChar,humanRow,humanCol);

	fclose(fp); // close file
	printf("\n\n**************************saving current state of game**********************\n");
	printf("saved current game state to 'savedTicTacToeGame.txt'\n");
...
int startTicTacToe () {
  ...
  int doesAddCross = 1;
	printf("the size of board should greater than 3 and less than 11\n");
  printf("Enter the size of field: \n");
	do {
		scanf("%d",&sizeF);	
	}while(sizeF >9 || sizeF < 4);
  ...
  do{
     ...
     while(isSolved == 0){     
          ...
          checkAdding(doesAddCross,sizeF,board);
				...
				//ask user if want to save current state	
				int count = 0;
				char answer= ' ';			
				printf("\nDo you want to save current state of game?\n");
				printf("input '1' -------------saving current state\n");
				printf("input '2' ------------continue game without saving\n");
				while(getchar() != '\n');//clear the buffer of input
    			do{	
      			count = 0;
     			answer = getchar();
     			while(getchar() != '\n'){count++;}
      			if((answer !='1' && answer != '2' )|| count>0){
       				 printf("re-Enter a valid character: \n");
      				}
    			}while((answer !='1'&& answer != '2')|| count>0);
     		if(answer == '1') {
     			saveTicGame(sizeF,humanChar,insertPositionRow,insertPositionCol);
     		}else if(answer == '2') {
					printf("don't save game, keep going\n");     		
     		}
